...
Or using cmake:
```bash
cmake -G Ninja -B build && cmake --build build && ./build/hello_reflection
```
Gotchas: 
- nonstatic_data_members_of(...) returns a std::vector, so you must wrap it in std::define_static_array(...) to iterate with template for (as in the smoke test). 
- The link-time libunwind.1.dylib warning is benign.
